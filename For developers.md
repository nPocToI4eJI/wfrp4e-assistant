...
- **opponent** _(Object)_: данные противника _(необязательный параметр)_. Укажите "false", для использования только Общих реакций, не привязанных к конкретным Народам.
...
Пример: `game.wfrp4e.utility.getReaction({name: "Имя", species: "Human", tokens: [...токены], to: "Arm"}, {species: "Goblin"}, "takeDamage")` создаст облако чата над указанными токенами, в котором будет случайно выбранная из списка реакция. Список будет состоять из реакций для _Human_, против оппонента _Goblin_, для действия _Получение урона_, по части тела _Рука_.
