class assistantUtility {
	...
	static async getReaction(actor, opponent, action) {
		if (actor.tokens[0] && actor.species && actor.species != "Not" && !game.wfrp4e.assistant.bubbles.includes(actor.id)) {
			...
			if (actor.subspecies && actor.subspecies != "Not" && phrases.hasOwnProperty(actor.subspecies)) {
			...
			if (opponent) {
				...
				if (opponent.species && opponent.species != "Not" && phrases.Opponent.All.hasOwnProperty(opponent.species)) {
					reactionsList = reactionsList.concat(Object.values(phrases.Opponent.All[opponent.species]?.All || []));
					if (opponent.subspecies && opponent.subspecies != "Not" && phrases.Opponent.All[opponent.species].hasOwnProperty(opponent.subspecies)) {
						reactionsList = reactionsList.concat(Object.values(phrases.Opponent.All[opponent.species][opponent.subspecies] || []));
					};
				};

				if (actor.subspecies && actor.subspecies != "Not" && phrases.Opponent.hasOwnProperty(actor.subspecies)) {
					reactionsList = reactionsList.concat(Object.values(phrases.Opponent[actor.subspecies]?.All || []));
					if (opponent.species && opponent.species != "Not" && phrases.Opponent[actor.subspecies].hasOwnProperty(opponent.species)) {
						reactionsList = reactionsList.concat(Object.values(phrases.Opponent[actor.subspecies][opponent.species]?.All || []));
						if (opponent.subspecies && opponent.subspecies != "Not" && phrases.Opponent[actor.subspecies][opponent.species].hasOwnProperty(opponent.subspecies)) {
			...
			if (actor.to && phrases?.To.hasOwnProperty(actor.to)) {
				...
				if (actor.subspecies && actor.subspecies != "Not" && phrases.To[actor.to].hasOwnProperty(actor.subspecies)) {
			...
			if (opponent.to && phrases?.To.hasOwnProperty(opponent.to)) {
				...
				if (phrases.To[opponent.to].hasOwnProperty(opponent.species)) {
					reactionsList = reactionsList.concat(Object.values(phrases.To[opponent.to][opponent.species].All || []));
					if (opponent.subspecies && opponent.subspecies != "Not" && phrases.To[opponent.to][opponent.species].hasOwnProperty(opponent.subspecies)) {
						reactionsList = reactionsList.concat(Object.values(phrases.To[opponent.to][opponent.species][opponent.subspecies] || []));
					};
				};
			};
...
Hooks.once("init", function () {
	...
	game.settings.register("wfrp4e-assistant", "translateImportedActors", {
		...
		scope: "world",
		config: game.modules.get("babele")?.active ? true : false,
		default: true,
		type: Boolean
	});
...
//Создание токена
"YRJEOMjZZ7iinnPx":
`if (!this.actor.inCompendium && game.settings.get("wfrp4e-assistant", "enableHelpers")) {
	...
	game.wfrp4e.assistant.bubbles.push(this.actor.uuid);
	setTimeout(() => {
		let index = game.wfrp4e.assistant.bubbles.indexOf(this.actor.uuid);
		if (index > -1) {game.wfrp4e.assistant.bubbles.splice(index, 1)};
	}, 1000);
	this.effect.update({
...
async function getEffect(actor, html) {
	...
	if (game.user.isGM && !actor.inCompendium) {
		if (game.settings.get("wfrp4e-assistant", "enableHelpers")) {
			...
			//Обновление до 13.2.5
			} else if (!effect.flags.assistant.version) {
				effect.update({
					"system.scriptData": effect.system.scriptData.map(s => {
						s.script = "if (game.modules.get(\"wfrp4e-assistant\")?.active) {" + s.script + "}";
						return s;
					}),
					"flags.assistant.version": game.modules.get("wfrp4e-assistant").version
				});
			//Включение эффекта
			} else {effect.update({
				"disabled": false,
