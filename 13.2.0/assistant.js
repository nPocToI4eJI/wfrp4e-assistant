class assistantUtility {
	...
	static async getReaction(actor, opponent, action) {
		let phrases = game.i18n.translations.WFRP4E.Assistant.Helpers.Reactions.List[actor.species];
		let reactionsList = Object.values(phrases[action].All || []);

		if (opponent) {
			reactionsList = reactionsList.concat(Object.values(phrases[action].Opponent.All || []));
			if (opponent.species != "Not") {
				reactionsList = reactionsList.concat(Object.values(phrases[action].Opponent[opponent.species] || []));
			};
		};

		if (actor.to) {reactionsList = reactionsList.concat(Object.values(phrases[action].To[actor.to] || []))};
		if (opponent.to) {reactionsList = reactionsList.concat(Object.values(phrases[action].To[opponent.to] || []))};
...
`if (game.modules.get("wfrp4e-assistant") && !this.actor.inCompendium && game.settings.get("wfrp4e-assistant", "enableHelpers")) {
	...
	if (params.generateName?.status && !this.actor.prototypeToken.actorLink) {
		let name = "";
		...
		speciesKeys.forEach(k => {
			...
			name += values[Math.floor(CONFIG.Dice.randomUniform() * values.length)];
		});
...
`if (game.modules.get("wfrp4e-assistant") && !this.actor.inCompendium && game.settings.get("wfrp4e-assistant", "enableHelpers")) {
	...
	if (args.data?.system?.status?.wounds && params.deathTint.color != params.deathTint.defaultColor) {
		let color;
		if (this.actor.status.wounds.value <= 0) {
			...
			color = params.deathTint.defaultColor
			if (game.modules.get("healthEstimate") && game.healthEstimate?.NPCsJustDie && this.actor.type != "character" && this.actor.hasCondition("dead")) {
				await this.actor.removeCondition("dead");
			};
		};

		if (!this.actor.prototypeToken.actorLink) {this.actor.token.update({"texture.tint": color})}
		else {
	...
	//Реакции. Получение урона. Отключено. Временно.
	/*
	if (params.reactions.status) {
		if (args.options.deltaWounds < 0) {
			console.log(args)
			let actor = {name: this.actor.name, species: params.reactions.species};
			...
			game.wfrp4e.utility.getReaction(actor, false, action);
		};
	};
	*/
};`,
...
`if (game.modules.get("wfrp4e-assistant") && !this.actor.inCompendium && game.settings.get("wfrp4e-assistant", "enableHelpers")) {
	...
	this.actor.characteristics.ws.value += parseInt(params.changeCharacteristics.value);
	this.actor.characteristics.bs.value += parseInt(params.changeCharacteristics.value);
	this.actor.characteristics.s.value += parseInt(params.changeCharacteristics.value);
	this.actor.characteristics.t.value += parseInt(params.changeCharacteristics.value);
	this.actor.characteristics.i.value += parseInt(params.changeCharacteristics.value);
	this.actor.characteristics.dex.value += parseInt(params.changeCharacteristics.value);
	this.actor.characteristics.ag.value += parseInt(params.changeCharacteristics.value);
	this.actor.characteristics.int.value += parseInt(params.changeCharacteristics.value);
	this.actor.characteristics.wp.value += parseInt(params.changeCharacteristics.value);
	this.actor.characteristics.fel.value += parseInt(params.changeCharacteristics.value);

	for (let skill of this.actor.itemTypes.skill) {
		skill.system.total.value += parseInt(params.changeCharacteristics.value);
	};
};`,
...
`if (game.modules.get("wfrp4e-assistant") && !this.actor.inCompendium && game.settings.get("wfrp4e-assistant", "enableHelpers")) {
	...
	if (params.reactions.status) {
		if (parseInt(params.reactions.frequency) >= (await new Roll("1d100").roll()).result) {
			let actor = {name: this.actor.name, species: params.reactions.species};
			actor.tokens = [this.actor.token] || this.actor.getDependentTokens().filter(t => t.parent == game.scenes.current);
			let attacker = {species: "Not"};
			let attackerEffect = args.attacker.effects.find(e => e.flags.assistant);
			if (attackerEffect) {
				attacker.species = attackerEffect.flags.assistant.reactions.species || "Not";
			};
			...
			game.wfrp4e.utility.getReaction(actor, attacker, action);
		};
	};
};`,
...
`if (game.modules.get("wfrp4e-assistant") && !this.actor.inCompendium && game.settings.get("wfrp4e-assistant", "enableHelpers")) {
	...
	if (params.reactions.status) {
		if (parseInt(params.reactions.frequency) >= (await new Roll("1d100").roll()).result) {
			let actor = {name: this.actor.name, species: params.reactions.species};
			actor.tokens = [this.actor.token] || this.actor.getDependentTokens().filter(t => t.parent == game.scenes.current);
			let target = {species: "Not"};
			let targetEffect = args.actor.effects.find(e => e.flags.assistant);
			if (targetEffect) {
				target.species = targetEffect.flags.assistant.reactions.species || "Not";
			};
			...
			game.wfrp4e.utility.getReaction(actor, target, action);
		};
	};
};`,
//Реакции. Защита
	...
	return await actor.createEmbeddedDocuments("ActiveEffect", [{
		...
		system: {
			scriptData: [
				...
				{
					label: game.i18n.localize("WFRP4E.Assistant.Helpers.Scripts.ComputeCharacteristics"),
					trigger: "computeCharacteristics",
					script: `[Script.pvLOGb5Nh4ZhHzKY]`
				},
				{
...
async function getEffect(actor, html) {
	let effect = actor.effects.filter(e => e.flags.assistant);
	...
	if (game.user.isGM && !actor.inCompendium) {
		if (game.settings.get("wfrp4e-assistant", "enableHelpers")) {
			...
			} else if (effect.scripts.length != 7 || !effect.scripts.some(s => s.trigger == "createToken") || !effect.scripts.some(s => s.trigger == "updateDocument") || !effect.scripts.some(s => s.trigger == "computeCharacteristics") || !effect.scripts.some(s => s.trigger == "takeDamage") || !effect.scripts.some(s => s.trigger == "applyDamage") || !effect.scripts.some(s => s.trigger == "opposedAttacker") || !effect.scripts.some(s => s.trigger == "opposedDefender")) {
				...
				switch (actor.type) {
					case "character": {
						params = {
							deathTint: {
								...
								defaultColor: params.deathTint?.defaultColor || game.settings.get("wfrp4e-assistant", "assistantPreset").character.deathTint.defaultColor
							},
							changeCharacteristics: {value: params.changeCharacteristics?.value || game.settings.get("wfrp4e-assistant", "assistantPreset").character.changeCharacteristics.value},
							spellsFilter: {
					...
					case "npc": {
						params = {
							...
							changeCharacteristics: {value: params.changeCharacteristics?.value || game.settings.get("wfrp4e-assistant", "assistantPreset").npc.changeCharacteristics.value},
							randomCharacteristics: {status: params.randomCharacteristics?.status || game.settings.get("wfrp4e-assistant", "assistantPreset").npc.randomCharacteristics.status},
							deathTint: {
								color: params.deathTint?.color || game.settings.get("wfrp4e-assistant", "assistantPreset").npc.deathTint.color,
								defaultColor: params.deathTint?.defaultColor || game.settings.get("wfrp4e-assistant", "assistantPreset").npc.deathTint.defaultColor
							},
							generateSpells: {
							...
							disposition: params.disposition || game.settings.get("wfrp4e-assistant", "assistantPreset").npc.disposition,
							spellsFilter: {
					...
					case "creature": {
						params = {
							...
							changeCharacteristics: {value: params.changeCharacteristics?.value || game.settings.get("wfrp4e-assistant", "assistantPreset").creature.changeCharacteristics.value},
							randomCharacteristics: {status: params.randomCharacteristics?.status || game.settings.get("wfrp4e-assistant", "assistantPreset").creature.randomCharacteristics.status},
							deathTint: {
								color: params.deathTint?.color || game.settings.get("wfrp4e-assistant", "assistantPreset").creature.deathTint.color,
								defaultColor: params.deathTint?.defaultColor || game.settings.get("wfrp4e-assistant", "assistantPreset").creature.deathTint.defaultColor
							},
							generateSpells: {
							...
							disposition: params.disposition || game.settings.get("wfrp4e-assistant", "assistantPreset").creature.disposition,
							spellsFilter: {
	...
	if (game.settings.get("wfrp4e-assistant", "hideHelpersTrait")) {
		actor.sheet.form.querySelector(`section[data-tab="effects"]>.effect-lists>.sheet-list>.list-content>div[data-uuid="${effect.uuid}"]`).style.display = "none";
	};
...
async function assistantMenu(actor) {
			...
			callback: (html) => {
				...
				if (actor.type == "character") {
					newParams = {
						...
						changeCharacteristics: {value: html.find("[id=changeCharacteristics]")[0].value},
						spellsFilter: {
							page: params.spellsFilter.page,
							list: params.spellsFilter.list,
							petty: params.spellsFilter.petty
						},
						reactions: {
							status: html.find("[id=reactionsStatus]")[0].checked,
							species: html.find("[id=reactionsSpecies]")[0].value,
							frequency: html.find("[id=reactionsFrequency]")[0].value
						}
					};
	...
	new Dialog({
		title: actor.name,
		content: await foundry.applications.handlebars.renderTemplate("modules/wfrp4e-assistant/templates/assistantMenu.hbs", {params, lores, species: Object.keys(game.i18n.translations.WFRP4E.Assistant.Helpers.GenerateName.List), reactions: Object.keys(game.i18n.translations.WFRP4E.Assistant.Helpers.Reactions.Species).map(s => ({value: s, name: game.i18n.translations.WFRP4E.Assistant.Helpers.Reactions.Species[s]}))}),
		buttons: buttons,
		default: "save",
		close: () => {}
