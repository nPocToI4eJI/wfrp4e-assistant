...
- [Методы](#Методы)
  ...
  - [getSpeciesKey](#getSpeciesKeystring)
- [Макросы](#Макросы)
...
## Исправления
<details>
<summary>Здесь перечислены исправления и доработки системы и официальных модулей.</summary>
...
Пример: `game.wfrp4e.utility.getMagicLoreKey("Знахарство")` вернёт `"hedgecraft"`.
...
Пример: `game.wfrp4e.utility.findAllSpells("hedgecraft", true)` вернёт массив, содержащий все заклинания Школы Магии Знахарства и общие заклинания (заклинания без указания Школы Магии).
...
Пример: `game.wfrp4e.utility.getSpeciesKey("Человек (Рейкландец)")` вернёт `"human"`.
...
- [ ] Переделать само состояние Защитной стойки, как минимум, выдав ему иконку.
...
- [ ] Добавить стандартные настройки Помощника для каждого типа актёра в настройках.
- [ ] Добавить функцию Помощника, которая при спавне будет запрашивать "Отношение" токена.
...
- [ ] Настройка **"Включить Помощника"** в выключенном состоянии отключает только возможность настройки Помощника, при этом настроенные скрипты продолжают функционировать.
