...
"BJmxXK4ESSTuenLd":
`let params = this.effect.flags.assistant;
if (args.data?.flags?.assistant && args.item == this.item) {
	/*
	this.actor.update({
	...
	*/
} else if (args.data?.system?.status?.wounds && params.deathTint.color != params.deathTint.defaultColor) {
	async function setCondition(target, dead) {
		if (game.healthEstimate?.NPCsJustDie && target.type != "character") {
			if (dead) {if (!target.hasCondition("dead")) {await target.addCondition("dead")}}
			else {await target.removeCondition("dead")};
		};
	};
	let color;
	if (this.actor.status.wounds.value <= 0) {
		color = params.deathTint.color;
		setCondition(this.actor, true);
	} else {
		color = params.deathTint.defaultColor;
		setCondition(this.actor, false);
	};
	if (!this.actor.prototypeToken.actorLink) {
		this.actor.token.update({"texture.tint": color});
	} else {
